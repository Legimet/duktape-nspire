...
/* Cleanup and exit */
_Noreturn void cleanup(duk_context *ctx, int retval) {
#ifdef DUK_CMDLINE_AJSHEAP
...
duk_ret_t exit_duktape(duk_context *ctx) {
    if (duk_is_null_or_undefined(ctx, 0)) {
	    cleanup(ctx, 0);
    } else {
	cleanup(ctx, duk_require_int(ctx, 0));
    }
}
...
int main(int argc, char *argv[]) {
	...
	cfg_register_fileext("js", "duktape");
